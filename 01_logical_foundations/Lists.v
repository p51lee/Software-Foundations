...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1, s2 with
  | nil, _ => true
  | _, h :: t => included (remove_one h s1) t
  | _, nil => false
  end.
...
Theorem add_inc_count : forall v: nat, forall l: natlist,
count v l + 1 = count v (v :: l).
Proof.
  intros v l.
  simpl.
...
Theorem bag_count_sum: forall (s1 s2: bag),
  count 0 (s1 ++ s2) = (count 0 s1) + (count 0 s2).
Proof.
  intros s1 s2.
  induction s1 as [|h t].
  - simpl. reflexivity.
